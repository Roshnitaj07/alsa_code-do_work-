...
CML_VOID setalsaconfig(scmlplayer *spsetconfig)
{
    ...
    printf("swaveheaderinfo.uwchannel: %i ", spsetconfig->spfileinfo->tmp);

    if (spsetconfig->spfileinfo->tmp == 1)
        printf("(mono)\n");
    else if (spsetconfig->spfileinfo->tmp == 2)
        printf("(stereo)\n");

    snd_pcm_hw_params_get_rate(spsetconfig->spalsaconfig->params, &spsetconfig->spfileinfo->tmp, 0);
    printf("swaveheaderinfo.isamplerate: %d bps\n", spsetconfig->spfileinfo->tmp);

    printf("swaveheaderinfo.iduration: %d\n", spsetconfig->spwavheaderinfo->iduration);	
